...
- `(Phase3BDA ∨ MEA)` precedes `RAR` (RAR consumes both)
...
| Phase3BDA | Phase2BDAReport, plus broader theater intelligence context | Phase3BDAReport |
| MEA | PostStrikeImagery, weapon-employment data | MEAReport |
| RAR | Phase3BDAReport ∨ MEAReport | RARReport |
...
1. **All §3 classes axiomatized** in `src/ontology/jio-core.ttl` (or its successor module structure, at Onto's discretion under JI-005).
...
3. **All §3.7 has_input / has_output relations** encoded as `someValuesFrom` restrictions; verified by competency tests.
4. **The five Combat Assessment report types** correctly classified as sub-classes of `IAO_0000030 (ICE)` under `ex:Report`, never inferable as `BFO_0000015 (Process)` — verified by the Tester's `ice_confusion_test` adversarial probe ([tests/fixtures/adversarial/ice_confusion_test/](../tests/fixtures/adversarial/ice_confusion_test/)).
5. **Anti-rigidity satisfied:** `ex:IntelligenceAnalyst` and `ex:TargetAnalyst` are not sub-classes of `cco:Person` (i.e., `ont00001262`); the relationship is `cco:Person bearer_of some ex:TargetAnalysisRole` per the JI-005 anti-rigidity refactor. Verified by `role_stress_test` adversarial probe.
