...
**Expected behavior:** The reasoner must NOT infer that `Process_Phase2BDA_001` is a valid Phase 2 BDA in this state. Specifically: if Phase 2 is axiomatized as `subClassOf preceded_by some Phase1BattleDamageAssessment`, then a Phase 2 individual without that predecessor should fail to satisfy the existential restriction. The reasoner must not silently accept the orphaned Phase 2.

**Tester routing:** [tests/fixtures/adversarial/temporal_consistency_test/](../../tests/fixtures/adversarial/temporal_consistency_test/). Tester translates this variant into a fixture file under that probe.
...
For Onto:
- **Q-O-1:** During IRI-fication, do you want me (SME) to author 2 additional CA scenarios (variant strike outcomes, multi-target) before JI-005 enters AXIOMATIZED, or is one canonical scenario sufficient for the first axiomatization pass and additional scenarios land post-JI-005?
- **Q-O-2:** The single-role-realized-in-multiple-processes design choice (§3.2) — confirm this is your preferred encoding under the anti-rigidity refactor, vs. minting a separate role individual per process. I lean toward single-role; this matches the doctrinal reality of an analyst owning the BDA chain.

For Tester:
- **Q-T-1:** §9.1 and §9.2 adversarial variants are described narratively. Do you want SME to also draft minimal A-Box snippets for them, or do you prefer to author the fixtures from scratch given the narrative? Per scenario template, the fixture is your lane regardless — just asking about the input format that's most useful for your translation.
